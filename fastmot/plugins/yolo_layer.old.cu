...
namespace nvinfer1
{
    ...
    Dims YoloLayerPlugin::getOutputDimensions(int index, const Dims* inputs, int nbInputDims)
    {
        ...
        std::cout << inputs[0].d[0] << ", " << inputs[0].d[1] << ", " << inputs[0].d[2] << ", " << inputs[0].d[3] << std::endl;
        assert(inputs[0].d[0] == mNumAnchors);
        assert(inputs[0].d[1] == mYoloHeight);
        assert(inputs[0].d[2] == mYoloWidth);
        assert(inputs[0].d[3] == (mNumClasses + 5));
    ...
    //     det->bbox[0] = (col - 0.5f + 2.0f * sigmoidGPU(cur_input[idx + 0 * total_grids])) / yolo_width;      // [0, 1]
    //     det->bbox[1] = (row - 0.5f + 2.0f * sigmoidGPU(cur_input[idx + 1 * total_grids])) / yolo_height;     // [0, 1]
    //     det->bbox[2] = 2.0f * sigmoidGPU(cur_input[idx + 2 * total_grids]);
    //     det->bbox[2] = det->bbox[2] * det->bbox[2] * anchors[2 * anchor_idx] / input_w;                      // [0, 1]
    //     det->bbox[3] = 2.0f * sigmoidGPU(cur_input[idx + 3 * total_grids]);
    //     det->bbox[3] = det->bbox[3] * det->bbox[3] * anchors[2 * anchor_idx + 1] / input_h;                  // [0, 1]
